...
function formatUsuario(user) {
  return {
    ...
    aceitou_termos_em: user.aceitou_termos_em,
  };
}

router.post('/register', async (req, res) => {
  const { email, username, senha, dataNascimento, aceitouTermos } = req.body;
  ...
  if (aceitouTermos !== true) {
    return res.status(400).json({ erro: 'É necessário aceitar os Termos e Condições para se cadastrar.' });
  }
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (email, username, senha_hash, data_nascimento, aceitou_termos_em)
       VALUES ($1, $2, $3, $4, now())
...
router.get('/me', requireAuth, async (req, res) => {
  const userResult = await pool.query(
    'SELECT id, email, username, created_at, aceitou_termos_em FROM users WHERE id = $1',
    [req.userId]
  );
  const user = userResult.rows[0];
